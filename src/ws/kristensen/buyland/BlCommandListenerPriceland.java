                ...
                if (player.hasPermission("buyland.price") || player.hasPermission("buyland.all")) {
                    ...
                    if (protectedRegion == null) {
                        ...
                        if (plugin.getRentConfig().contains("rent." + argRegionName + ".rentable")) {
                            //TODO: send this to the rentland cost function instead of doing it here since they may not have rights for rent.
                            Bukkit.dispatchCommand(sender, "rentland cost " + argRegionName);
                        } else {
                            //See if the region if buyable
                            if (!isRegionBuyable) {
                                plugin.sendMessageInfo(sender, ChatColor.translateAlternateColorCodes('&', plugin.getLanguageConfig().getString("buyland.price.dontown")));
                            } else {
                                ...
                                plugin.sendMessageInfo(sender, ChatColor.translateAlternateColorCodes('&', plugin.getLanguageConfig().getString("buyland.price.cost")) + regionPrice);
                                ...
                                plugin.sendMessageInfo(sender, String.format(ChatColor.translateAlternateColorCodes('&', plugin.getLanguageConfig().getString("buyland.price.price")), BuyLand.econ.format(economyResponse.balance)));
                                ...
                                int maximumNumberRegionsPlayerCanOwn = plugin.getMaxNumberOfRegionsPlayerCanOwn(player);

                                //Notify the player of their current regions and max regions.
                                plugin.sendMessageInfo(sender,
                                                       ChatColor.translateAlternateColorCodes('&', plugin.getLanguageConfig().getString("buyland.price.max1"))
                                                       + currentNumberPlayerOwnedRegions +
                                                       ChatColor.translateAlternateColorCodes('&', plugin.getLanguageConfig().getString("buyland.price.max2"))
