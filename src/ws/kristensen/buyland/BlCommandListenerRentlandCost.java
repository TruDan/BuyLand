                ...
                if (player.hasPermission("buyland.rent") || player.hasPermission("buyland.all")) {    
                    ...
                    if (regionManager.getRegionExact(argRegionName) == null) {
                        ...
                        if (plugin.rentGetConfig().contains("rent." + argRegionName + ".rentable")) {
                            //this is rentland
                            double s = plugin.rentGetConfig().getDouble("rent." + argRegionName +".costpermin") / 2;
                            double m = plugin.rentGetConfig().getDouble("rent." + argRegionName +".costpermin");
                            double h = plugin.rentGetConfig().getDouble("rent." + argRegionName +".costpermin") * 60;
                            double d = plugin.rentGetConfig().getDouble("rent." + argRegionName +".costpermin") * 60 * 24;
                            double w = plugin.rentGetConfig().getDouble("rent." + argRegionName +".costpermin") * 60 * 24 * 7;
